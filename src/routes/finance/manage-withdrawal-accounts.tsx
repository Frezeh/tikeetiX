...
export default function ManageWithdrawalAccounts() {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: deleteSettlementAccount,
  });
  ...
  return (
    <div className="w-full h-screen overflow-hidden">
      ...
      {data?.data &&
        data.data.map((d) => (
          <div
            className="px-5 pt-5 py-6 border-b border-[#E4E7EC] flex justify-between items-start"
            key={d.id}
          >
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="flex items-center gap-1">
                    ...
                    <p className="text-xs text-[#0D67FE] font-medium">
                      Manage account
                    </p>
                  </button>
                </DropdownMenuTrigger>
                  ...
                  >
                    <span className="flex items-center gap-[10px] text-[#BD1B0F] text-xs">
                      {isPending ? (
                        ...
                        <>
                          <BinIcon fill="#BD1B0F" width={20} height={20} />{" "}
                          Cancel ticket
                        </>
                      )}
                    </span>

...
export default function RefundedTickets({ amount }: { amount: number }) {
  const [refundedTicketDate, setRefundedTicketDate] = useState(
  ...
  const { isLoading: isLoadingYearlyReport, data: YearlyReport } = useQuery({
    queryKey: [`yearly-report-refunded-${refundedTicketDate}`],
    queryFn: () =>
      getYearlyReport(Number(refundedTicketDate), "Refunded", "Events"),
  });
    ...
    <Card className="border-none w-full shadow-none">
      <CardHeader>
        <CardDescription className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Select onValueChange={setRefundedTicketDate}>
              <SelectTrigger
                prefixIcon={
                  <CalendarIcon

...
export default function AllEvents(props: Props) {
  ...
  const queryParams = useMemo(() => {
    ...
    if (allEventsFilterValue) {
      params = params + `&status=${allEventsFilterValue}`;
    }
    if (allEventsSearchValue) {
      params = params + `&searchTerm=${allEventsSearchValue}`;
    }
          ...
          <Card className="border-[#E4E7EC] w-[300px] h-[84px] p-3 rounded-[12px] mx-5">
            <CardContent className="flex justify-between items-center p-0">
              <div className="space-y-2">
                <p className="text-[#475367] text-sm">Total no. of tickets</p>
      ...
      <div className="border-t border-[#E4E7EC] w-full my-4 py-5 px-3 space-y-4">
        <div className="flex items-center">
          ...
          <div className="ml-auto flex items-center gap-2">
            <Input
              value={allEventsSearchValue}
              onChange={(e) => setAllEventsSearchValue(e.target.value)}
              ...
              >
                <div className="py-3 px-2 grid grid-cols-3 justify-center items-center border-b border-b-[#F0F2F5] bg-[#F0F2F5]">
                  <button
                    ...
                    onClick={() => setAllEventsFilterValue("")}
                  >
                    Clear
                  </button>
                ...
                <div className="grid grid-cols-3 px-1 pb-4">
                  <div className="space-y-1">
                    <FilterItem
                      checked={allEventsFilterValue === "DRAFT"}
                      onCheckedChange={() => setAllEventsFilterValue("DRAFT")}
                    >
                      Draft
                    </FilterItem>
                    <FilterItem
                      checked={allEventsFilterValue === "PENDING"}
                      onCheckedChange={() => setAllEventsFilterValue("PENDING")}
                    >
                      Pending
                    </FilterItem>
                  </div>
                  <div className="space-y-1">
                    <FilterItem
                      checked={allEventsFilterValue === "PAUSED"}
                      onCheckedChange={() => setAllEventsFilterValue("PAUSED")}
                    >
                      Paused
                    </FilterItem>
                    <FilterItem
                      checked={allEventsFilterValue === "ONGOING"}
                      onCheckedChange={() => setAllEventsFilterValue("ONGOING")}
                    >
                      Ongoing
                    </FilterItem>
                  </div>
                  <div className="space-y-1">
                    <FilterItem
                      checked={allEventsFilterValue === "COMPLETED"}
                      onCheckedChange={() =>
                        setAllEventsFilterValue("COMPLETED")
                      }
                    >
                      Completed
                    </FilterItem>
